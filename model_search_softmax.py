...
class Network(nn.Module):
    ...
    def forward(self, input, epochs=150, arch=None):
        ...
        # print torch.sum(weights_norm > 0)
        # print torch.sum(weights_reduce > 0)
    ...
    def genotype2weight(self, arch):
        ...
        # weights_normal = weights_normal[:, :, np.newaxis]
        # weights_reduce = weights_reduce[:, :, np.newaxis]
        #
        # weights_normal = np.tile(weights_normal, (1, 1, 96))
        # weights_reduce = np.tile(weights_reduce, (1, 1, 96))
